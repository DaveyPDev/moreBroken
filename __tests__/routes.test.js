...
describe('POST /auth/login', function () {
	test('should allow a correct username/password to log in', async function () {
		...
		expect(response.body).toEqual({ message: "Logged in!", token: expect.any(String) });
...
describe('GET /users', function () {
	test('should deny access if no token provided', async function () {
		...
		const response = await request(app).get('/users');
		expect(response.statusCode).toBe(200);
		expect(response.body.users).toContainEqual({ username: 'u1', first_name: 'fn1', last_name: 'ln1' });
		expect(response.body.users).toContainEqual({ username: 'u2', first_name: 'fn2', last_name: 'ln2' });
		expect(response.body.users).toContainEqual({ username: 'u3', first_name: 'fn3', last_name: 'ln3' });
	});
...
describe('PATCH /users/[username]', function () {
	test('should deny access if no token is provided', async function () {
...
describe('DELETE /users/[username]', function () {
	...
	test('should deny access if not admin', async function () {
		const response = await request(app).delete('/users/u1').query({ _token: tokens.u1 });
		expect(response.statusCode).toBe(401);
	});
